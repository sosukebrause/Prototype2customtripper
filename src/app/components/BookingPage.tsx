...
import { MapPin, User, Briefcase, Calendar, DollarSign, Phone, Mail } from "lucide-react";
...
export function BookingPage() {
  ...
  const calculateTotalPrice = () => {
    let total = 0;
    Object.entries(selectedVehicles).forEach(([vehicleId, quantity]) => {
      const vehicle = vehicles.find((v) => v.id === vehicleId);
      if (vehicle) {
        // Use full day rate if 5+ hours
        const rate = estimatedHours >= 5 ? vehicle.fullDayRate : vehicle.hourlyRate * estimatedHours;
        total += rate * quantity;
      }
    });
    return total;
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-4">
                      ...
                      <div key={index} className="flex items-start gap-3">
                        <div className="flex flex-col items-center">
                          <div className={`size-3 rounded-full ${
                            index === 0 ? "bg-green-500" : index === allLocations.length - 1 ? "bg-red-500" : "bg-blue-500"
                          }`} />
                        ...
                        <div className="flex-1">
                          ...
                          {index === 0 && <span className="text-xs text-gray-500">Starting point</span>}
                          {index === allLocations.length - 1 && <span className="text-xs text-gray-500">Final destination</span>}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex items-center gap-3 pt-4 border-t">
                  ...
                  <div>
                    ...
                    <div className="text-sm">{tripData.passengers} Passenger{tripData.passengers !== "1" ? "s" : ""}</div>
                  </div>
                </div>
              </div>
            </Card>
              ...
              <div className="relative bg-gray-100 rounded-lg h-80 flex items-center justify-center overflow-hidden">
                ...
                <svg width="100%" height="100%" viewBox="0 0 400 300" className="absolute inset-0">
                  ...
                  {allLocations.map((_, index) => {
                    const positions = allLocations.length === 2 
                      ? [{x: 100, y: 80}, {x: 300, y: 220}]
                      : allLocations.length === 3
                      ? [{x: 80, y: 100}, {x: 200, y: 180}, {x: 320, y: 240}]
                      : [{x: 60, y: 80}, {x: 180, y: 150}, {x: 280, y: 200}, {x: 340, y: 250}];
            ...
            <Card className="p-6">
              ...
              <div className="space-y-3">
                {Object.entries(selectedVehicles).map(([vehicleId, quantity]) => {
                  const vehicle = vehicles.find((v) => v.id === vehicleId);
                  ...
                  return (
                    <div key={vehicleId} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div className="flex-1">
                        <div className="font-medium">{vehicle.name}</div>
                        <div className="text-sm text-gray-600">
                          Quantity: {quantity} × ${rate.toFixed(2)}
                        </div>
                      </div>
            ...
            {/* Special Requests */}
            <Card className="p-6">
              <h2 className="text-lg font-semibold mb-4">Special Requests / Notes</h2>
              <textarea
              ...
              <p className="text-xs text-gray-500 mt-2">
                Optional: Let us know about child seats, wheelchair accessibility, stops, or any other requirements.
              </p>
            </Card>
            ...
            <Card className="p-6">
              ...
              <div className="text-sm text-gray-700 space-y-2 mb-4">
                ...
                <ul className="list-disc list-inside space-y-1 ml-2">
                  ...
                  <li>Additional charges may apply for route changes or extended hours</li>
                  <li>Payment information must be provided before final confirmation</li>
                  <li>Prices are subject to change until booking is finalized</li>
                </ul>
              </div>
          ...
          <div className="lg:col-span-1">
            <Card className="p-6 sticky top-6">
              ...
              <p className="text-xs text-gray-500 mb-4">
                * Final pricing will be confirmed after all mandatory details are provided
              </p>
              ...
              <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                <div className="flex items-start gap-2 mb-2">
                  ...
                  <div>
                    <div className="text-xs font-semibold text-blue-900">Questions or Comments?</div>
                    <div className="text-xs text-blue-700 mt-1">Contact our Sales Representative</div>
                  </div>
                </div>
                <div className="flex items-center gap-2 mt-2">
                  <Mail className="size-4 text-blue-600" />
                  <a href="mailto:sales@tripplanner.com" className="text-xs text-blue-600 hover:underline">
